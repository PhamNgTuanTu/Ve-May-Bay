...
import { saveDataHanhLyChuyenVe, tinhPhiVeChonGhe } from "store/actions";

const SelectFieldPhuTro = props => {
  const { name, placeholder, options, smallWidth, isClearable, id, loaiHanhKhach, value } = props;
  const dispatch = useDispatch()
  const { paramsPhiVe, arrHanhLyHaiChieu, arrPhuTroHaiChieu } = useSelector(state => ({
  ...
  const onChangeSelect = (option) => {
    ...
    dispatch(saveDataHanhLyChuyenVe(arr))
    findValue(value)
  }
