...
import { saveAfterArrSeatChooseMotChieu, saveNameSeatChooseMotChieu, tinhPhiVeChonGhe } from 'store/actions';
...
function PaneSeat(props) {
    ...
    const { arrSeatMotChieu, nameSeatMotChieu, paramsPhiVe, resApiPhiVe } = useSelector(state => ({
        arrSeatMotChieu: state.DatTimVe.arrSeatMotChieu,
        nameSeatMotChieu: state.DatTimVe.nameSeatMotChieu,
    ...
    const changeSeat = (e, price, vat, total, arrSeat) => {
        ...
        var length = arrSeatMotChieu.length;
        ...
        tinhPhiKhiChonGhe(paramsPhiVe, arrSeatMotChieu)
    }
    ...
    const updateValue = (name, value, checked, price, vat, total, arrSeat) => {
        if (index >= 0) {
            ...
            dispatch(saveAfterArrSeatChooseMotChieu(markers))
            // setArrSeatResult(markers)
        }
    }
    ...
    useEffect(() => {
        if (JSON.stringify(arrSeatMotChieu) !== "[]") {
            let arr = []
            for (let i = 0; i < arrSeatMotChieu.length; i++) {
                if (arrSeatMotChieu[i]["ten_ghe"] !== "") {
                    arr.push({ [arrSeatMotChieu[i]["ten_ghe"]]: arrSeatMotChieu[i]["status"] })
                }
            }
            let newObj = arr.reduce((a, b) => Object.assign(a, b), {})
            dispatch(saveNameSeatChooseMotChieu(newObj))
        }
    }, [arrSeatMotChieu])
    ...
    useEffect(() => {
        setCheckedItems(nameSeatMotChieu)
    }, [nameSeatMotChieu])
